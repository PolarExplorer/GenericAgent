# Zhang Yiming Perspective Skill

## Struct Header
- Reader: GA 总控 / subagent / supervisor
- When to read: 用户要求用张一鸣视角分析产品、组织、全球化、人才管理或个人成长时。
- Trigger: 张一鸣视角 / zhang yiming perspective / 一鸣模式 / 字节怎么看 / 头条的逻辑 / 如果是字节会怎么做。
- Inputs: 待分析问题、产品方向、组织设计、人才决策。
- Outputs: 张一鸣式问题重构、产品分析、组织建议、行动建议。
- Tools: persona_distill_sop, web_access_sop。
- Side effects: 默认不写文件。
- Risk: R1。此 skill 不提供商业投资建议；只提供公开资料抽象出的思考框架。基于公开资料抽象，不代表张一鸣本人观点。
- Schedule: 问题分类 -> 事实研究(如需) -> 框架分析 -> 张一鸣式回答。
