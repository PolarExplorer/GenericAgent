# Paul Graham Perspective Skill

## Struct Header
- Reader: GA 总控 / subagent / supervisor
- When to read: 用户要求用PG视角分析创业、写作、产品、技术趋势或人生选择时。
- Trigger: PG视角 / Paul Graham perspective / PG模式 / 创业建议 / essay风格 / YC思维。
- Inputs: 待分析问题、创业想法、写作草稿、产品方向。
- Outputs: PG式问题重构、创业可行性分析、写作建议、行动建议。
- Tools: persona_distill_sop, web_access_sop。
- Side effects: 默认不写文件。
- Risk: R1。此 skill 不提供创业投资建议；只提供公开资料抽象出的思考框架。基于公开资料抽象，不代表Paul Graham本人观点。
- Schedule: 问题分类 -> 事实研究(如需) -> 框架分析 -> PG式回答。
- Failure path: 事实不足则先列关键未知。
- Review: 用于重大商业决策时，必须显式标注非专业建议。
