...
- **需求/边界/验收标准不清时，禁止直接进入实施**；必须先在探索态补齐，再写计划
- **有多种可行方案且会影响后续实现时，必须先收敛方案**：主agent先给出 2~3 个可选方案/权衡，必要时 ask_user 确认，禁止边做边决定
- **分析瘫痪防护（来源：GSD executor 5分钟规则）**：探索/方案收敛阶段上限 3 轮工具调用。超过后必须基于已有信息选定方案开始执行，禁止无限探索。信息不完美是常态，80%确定性即可行动

**目标**：在写任何计划之前，搞清4件事：
① 环境现状（有什么、缺什么） ② 可用SOP ③ 关键不确定点 ④ 需求边界与验收标准

**为什么必须用subagent**：主agent上下文是最稀缺资源，探测长输出会挤占规划执行空间。

**核心思想**：探索态既是信息探测，也是“隔离实验区”。高噪音、高试错、高不确定任务优先放在 subagent 或独立工作目录中处理，避免污染主agent上下文和主执行路径。
...
进入下一阶段前，用 3~5 项可证据化条件做准入检查；未通过则补缺或 ask_user，禁止带病推进。

| Gate | 通过条件 | 不通过处理 |
|---|---|---|
| 探索→规划 | 环境现状/可用SOP/关键不确定点/验收标准已写入探索发现 | 补探测；边界不清则 ask_user |
| 规划→执行 | plan覆盖原始需求；依赖/风险/[D]/[VERIFY]齐全；用户已确认 | patch plan；多方案未收敛则 ask_user |
| 执行中变更 | 发现前提变化、步骤不可行或更优路径时，plan已Reverse Sync | 先 patch plan.md，再继续执行 |
| 执行→验证 | 所有执行步骤已标[✓]；交付物路径/检查项明确 | 补执行或补verify_context |
| 验证→收尾 | 独立验证有工具证据且VERDICT非BLOCKED；0个[ ]残留 | 按验证反馈修复，回到执行态 |
...
- □ 需求、边界、验收标准是否已经明确写入？（仍模糊 → 不得进入执行态）
- □ 若存在多方案，是否已完成方案收敛？（已选定/已ask_user确认，禁止边做边定）
- □ 每步的SOP标注是否合理？（SOP真的能解决该步？）
- □ 步骤间依赖是否正确？（有没有隐含依赖没写出来）
- □ 高风险步骤（SOP:无/不可逆）有没有清晰的执行思路？
- □ 步骤粒度是否合适？（禁止"处理所有文件"，必须展开具体条目）
- □ **复杂/繁琐步骤是否标注了[D]？**（读大量代码/网页/重复操作/高试错步骤必须委托subagent）
- □ **反 scope reduction 检查（来源：GSD planner）**：逐条对照用户原始需求，plan 是否覆盖了每一项？是否有功能被悄悄降级（真实实现→mock/stub/TODO）、边界情况被跳过、非功能需求（性能/错误处理/兼容性）被忽略？遗漏即补，不得以"后续再做"搪塞
- □ **是否包含"验证检查点"section，且有[VERIFY]步骤？（必须有，这是强制步骤）**
- □ **plan是否避免未验证完成宣称？**（所有“完成/修复/可用”表述都应指向后续验证步骤，而非提前下结论）
...
7. **标记完成** — `file_patch` 标记 `[ ]` → `[✓ 简要结果]`（进度写入plan.md）
8. **继续** — 立即回到步骤1，file_read(plan.md) 执行下一个 `[ ]`
...
- **禁止默默偏离plan**：实际执行与plan不符时，必须先 file_patch 修正 plan.md（Reverse Sync），再继续执行。偏离不改plan = 计划失效
...
### 🚦 Checkpoint 分级协议（来源：GSD executor 90/9/1 规则）

执行态中，按影响程度分级决定自主 vs 请示：

| 级别 | 占比 | 触发条件 | 行为 |
|------|------|---------|------|
| 自主 | ~90% | 文件读写、代码修改、测试运行、单文件重构、依赖安装（已在plan中） | 直接执行，标记[✓] |
| 决策点 | ~9% | 多方案选择、架构变更、新增依赖/工具引入、plan偏离需Reverse Sync、发现需求歧义 | ask_user 确认后执行 |
| 人工必须 | ~1% | 密钥/凭证操作、外部服务注册、不可逆删除、生产环境操作 | 停止执行，ask_user，等待明确指令 |

> plan中已经用户确认过的步骤视为「自主」级别，无需重复请示。决策点的核心判断标准：**这个决定错了能否低成本回退？** 能→自主，不能→请示。
