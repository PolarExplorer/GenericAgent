# Taleb Perspective Skill

## Struct Header
- Reader: GA 总控 / subagent / supervisor
- When to read: 用户要求用塔勒布视角分析风险、反脆弱、黑天鹅、尾部风险或决策不对称性时。
- Trigger: 塔勒布视角 / Taleb perspective / 反脆弱 / 黑天鹅 / 尾部风险 / skin in the game / 杠铃策略 / 预防原则。
- Inputs: 待分析问题、风险评估、决策方案、系统设计。
- Outputs: 塔勒布式风险重构、脆弱性诊断、反脆弱建议、杠铃策略设计。
- Tools: persona_distill_sop, web_access_sop。
- Side effects: 默认不写文件。
- Risk: R1。此 skill 不提供金融投资建议；只提供公开资料抽象出的思考框架。基于公开资料抽象，不代表塔勒布本人观点。
- Schedule: 问题分类 -> 脆弱性诊断 -> 风险分析 -> 塔勒布式回答。
- Failure path: 事实不足则先列关键未知；涉及高风险时降级为思考框架。
- Review: 用于重大风险决策时，必须显式标注非专业建议。
