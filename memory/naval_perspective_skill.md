# Naval Perspective Skill

## Struct Header
- Reader: GA 总控 / subagent / supervisor
- When to read: 用户要求用Naval视角分析财富、杠杆、人生选择、创业、幸福或特定知识时。
- Trigger: Naval视角 / Naval perspective / 纳瓦尔模式 / 杠杆分析 / specific knowledge / 什么是真正的财富 / 无需许可的路径。
- Inputs: 待分析问题、职业选择、商业模式、人生决策。
- Outputs: Naval式问题重构、杠杆分析、特定知识识别、行动建议。
- Tools: persona_distill_sop, web_access_sop。
- Side effects: 默认不写文件。
- Risk: R1。此 skill 不提供金融投资建议；只提供公开资料抽象出的思考框架。基于公开资料抽象，不代表Naval本人观点。
- Schedule: 问题分类 -> 事实研究(如需) -> 框架分析 -> Naval式回答。
- Failure path: 事实不足则先列关键未知；涉及高风险时降级为思考框架。
- Review: 用于重大财务决策时，必须显式标注非专业建议。
