...
FSAPP = os.path.join(BASE_DIR, 'frontends', 'fsapp.py')
LOG_FILE = os.path.join(BASE_DIR, 'temp', 'fsapp_daemon.log')
...
def run():
    ...
    while True:
        ...
        log.info(f'启动 fsapp.py (pid will follow)')
        try:
            proc = subprocess.Popen(
                [PYTHON, FSAPP],
                cwd=BASE_DIR,
                stdout=open(os.path.join(BASE_DIR, 'temp', 'fsapp_stdout.log'), 'a', encoding='utf-8'),
                stderr=subprocess.STDOUT,
                creationflags=0x08000000  # CREATE_NO_WINDOW
            )
            log.info(f'fsapp.py 已启动, PID={proc.pid}')
            proc.wait()
            code = proc.returncode
            log.warning(f'fsapp.py 退出, code={code}')
