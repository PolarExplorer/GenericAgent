...
def on_message(bot, msg):
    ...
    if text.startswith('/llm'):
        ...
        if len(args) > 1:
            try:
                n = int(args[1]); agent.next_llm(n); agent.baseline_llm_no = agent.llm_no; agent.llm_locked = True
                bot.send_text(uid, f'切换到 [{agent.llm_no}] {agent.get_llm_name()}（已锁定）', context_token=ctx)
